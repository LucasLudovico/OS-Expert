    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (metodo.equalsIgnoreCase("GET")) {
            ...
            if (body.contains("adicionaisAprovados")) {
                ...
                statusAtualOS = "PAGO";
                jsonResposta = "{\"status\":\"pago\",\"mensagem\":\"Pagamento registrado com sucesso!\"}";
            } else if (body.contains("finalizarMecanico")) {
                // --- INJEÇÃO DO FLUXO DO MECÂNICO: FINALIZAÇÃO DE ORDEM ---
                statusAtualOS = "EM_REVISAO";

                String laudoMecanico = extrairTextoJson(body, "observacoes");
                String tempoGastoStr = extrairTextoJson(body, "tempoTotal");

                System.out.println(">>> [OS ATUALIZADA PELO MECÂNICO]");
                System.out.println("Laudo Técnico: " + laudoMecanico);
                System.out.println("Tempo de Mão de Obra: " + tempoGastoStr);
                System.out.println("Novo Status da OS: " + statusAtualOS);

                jsonResposta = "{\"status\":\"sucesso\",\"mensagem\":\"OS enviada para a revisão final do atendente.\"}";
            } else {
                // Caso seja o payload inicial gerado pelo Atendente
                jsonResposta = "{\"status\":\"ok\"}";
            }
